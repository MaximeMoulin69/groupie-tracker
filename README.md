...
### Variables du sujet
- `scoreboardActualPointInGame` (colonne score)
- `const nbrs_manche = 9`
- Description : "Choisis une des playlist !"

### Fonctions clés
- `IsRoomReady(r Room) bool`
- `ValidatePseudo()` - Majuscule
- `ValidatePassword()` - CNIL
- `CalculateBlindTestPoints()` - Rapidité
- `CalculatePetitBacPoints()` - 0/1/2
- `GenerateRandomLetter()` - Aléatoire

### Commentaires IA (4 endroits)

J'ai utilisé l'IA pour comprendre :

1. **`auth/middleware.go`**
   - Pattern du middleware Go
   - Comment filtrer les requêtes

2. **`room/websocket.go`**
   - Pattern Hub pour WebSocket
   - Gestion centralisée des clients

3. **`game/petitbac.go`**
   - Génération lettre aléatoire
   - Utilisation de `rand.Seed` et `map`

4. **`main.go`**
   - Closures pour passer le hub
   - Fonctions anonymes en Go
